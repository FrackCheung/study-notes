...
***
**注解:** 关于`flex`和`flex-inline`
+ 设置为`flex`, 弹性容器呈现块级盒的行为, 宽度默认填满弹性容器宽度
+ 设置为`flex-inline`, 弹性容器呈现行内盒的行为, 不会独占一行
***
+ 弹性子元素: 弹性容器的直接子元素, 其他后代元素不受影响
+ 轴: 弹性子元素沿主轴方向排列, 超宽折行时, 新行沿副轴方向添加
***
...
+ 指定主轴: 使用`flex-direction`属性
  - `row`: 默认, 主轴为横向, 弹性子元素从左到右排列
  - `row-reverse`: 主轴为横向, 弹性子元素从右到左排列
  - `column`: 主轴为纵向, 弹性子元素从上到下排列
  - `column-reverse`: 主轴为纵向, 弹性子元素从下到上排列
***
**注解1:** 再次强调, 弹性子元素沿主轴排列, 需要折行时, 新行沿副轴方向添加
...
***
+ 折行: 使用`flex-wrap`属性
  - 默认情况下, 弹性子元素不会换行, 要么缩减尺寸, 要么溢出
  - `nowrap`: 默认, 不换行, 弹性子元素只在一行上排布
  - `wrap`: 超宽折行, 新行沿副轴方向添加
...
```
***
+ 创建一个支持换行的弹性布局, 弹性子元素从上往下排, 新行在前一行左侧
```CSS
div.flex {
  display: flex;
...
+ `flex-wrap`: 设置弹性子元素是否允许换行, 只能用于弹性容器
+ `flex-flow`: 一并设置`flex-direction`和`flex-flow`, 只能用于弹性容器
***

#### 弹性子元素分布和对齐
+ 弹性子元素默认向主轴起点靠近, 彼此之间没有间隔
+ 弹性子元素在主轴上的分布: 使用`justify-content`属性, 只能用于弹性容器
  - `flex-start`: 默认, 弹性子元素紧靠主轴起点分布
  - `flex-end`: 弹性子元素紧靠主轴终点分布
  - `center`: 弹性子元素整体居中分布
  - `space-between`: 两端弹性子元素紧靠起点终点, 剩余等间距分布
  - `space-around`: 所有弹性子元素等间距分布, 两侧空白为间距的一半
  - `space-evenly`: 所有弹性子元素等间距分布, 两侧空白和间距相等
***
**注解1:** 间距由弹性容器计算当前行抛开弹性子元素后的剩余宽度来决定

**注解2:** 外边距视为弹性子元素的一部分, 会一起扣除后再进行间距计算

**注解3:** 如果弹性容器设置`flex-wrap: nowrap`, 但弹性子元素超宽
+ `flex-start`: 弹性子元素在主轴终点溢出
+ `flex-end`: 弹性子元素在主轴起点溢出
+ `center`: 弹性子元素向两端溢出, 溢出量相等
+ `space-between`: 弹性子元素在主轴终点溢出
+ `space-around`: 弹性子元素向两端溢出, 溢出量相等
+ `space-evenly`: 弹性子元素向两端溢出, 溢出量相等
***
+ 弹性子元素在主轴上的对齐: 使用`align-items`属性, 只能用于弹性容器
  - `flex-start`: 同行所有弹性子元素顶端对齐
  - `flex-end`: 同行所有弹性子元素底端对齐
  - `center`: 同行所有弹性子元素水平居中对齐
  - `baseline`: 同行所有弹性子元素的基线对齐
  - `stretch`: 默认值, 同行所有弹性子元素被拉伸至同一高度
***
**注解1:** 关于`baseline`基线
+ 元素中第一行文字的底部就是该元素的基线, 可以理解为下划线位置
+ 基线对齐, 就是所有弹性子元素第一行文本的基线在同一条线上
```CSS
...
+ 拉伸高度包含弹性子元素的上下外边距, 如果设置外边距, 视觉上会短一点
```CSS
div.flex { display: flex; align-items: stretch; }
div:nth-child(2) { margin: 10px 0; }
```
***
+ 单个弹性子元素在主轴上的对齐: 使用`align-self`属性, 只能用于弹性子元素
...
+ `baseline`: 该弹性子元素的基线, 和同行中其他弹性子元素中最低的基线对齐
+ `center`: 该弹性子元素在该行中垂直居中

**注解2:** 行的高度, 是由该行中高度最大的弹性子元素决定的
***
+ 弹性子元素行在副轴上的对齐: 使用`align-content`属性, 只能用于弹性容器
  - `flex-start`: 弹性子元素行紧靠副轴起点分布
  - `flex-end`: 弹性子元素行紧靠副轴终点分布
  - `center`: 弹性子元素行整体居中分布
  - `space-between`: 两端弹性子元素行紧靠起点终点, 剩余等间距分布
  - `space-around`: 所有弹性子元素行等间距分布, 两侧空白为间距的一半
  - `space-evenly`: 所有弹性子元素行等间距分布, 两侧空白和间距相等
  - `stretch`: 默认, 拉伸各行, 填充所有高度, 将空白区域平均分配给各行
***
**注解1:** 使用`align-content`属性的前提是, 弹性容器具有明确的高度

**注解2:** 所有弹性子元素行超高时
+ `flex-start`: 弹性子元素行在副轴终点溢出
+ `flex-end`: 弹性子元素行在副轴起点溢出
+ `center`: 弹性子元素行向两端溢出, 溢出量相等
+ `space-between`: 弹性子元素行在副轴终点溢出
+ `space-around`: 弹性子元素行向两端溢出, 溢出量相等
+ `space-evenly`: 弹性子元素行向两端溢出, 溢出量相等
***
+ 本小节主要讲述了弹性子元素, 及其行的分布对齐, 内容较多
***
**总结:** 本小节介绍了如下CSS属性
+ `justify-content`: 同行弹性子元素在主轴上的分布方式, 只能用于弹性容器
+ `align-items`: 同行弹性子元素在主轴上的对齐方式, 只能用于弹性容器
+ `align-self`: 单个弹性子元素在所在行的对齐方式, 只能用于弹性子元素
+ `align-content`: 不同的弹性子元素行, 在纵轴上的对齐方式, 只能用于弹性容器
***

#### 弹性子元素
+ 关于弹性子元素的一些特征:
  - 弹性子元素的外边距不会折叠和溢出, 因为弹性容器创建了BFC
  - 弹性子元素和浮动元素一样, 不论初始类型是什么, 一律生成块级盒
  - 弹性子元素不能设置为浮动元素, 也不能设置清除浮动
  - 弹性容器中的注释和空白文本节点会被忽略, 其余元素和文本一律成为弹性子元素
```HTML
<style>
...
```
***
**注解:** `vertical-align`对于弹性子元素没有影响, 不能用于垂直居中
***
+ 弹性子元素可以通过绝对定位`position: absolute`移除文档流, 不再参与弹性布局
+ 绝对定位的弹性子元素, 其样式还是会受到一些影响
...
/** 该元素会在弹性容器中水平垂直居中 */
```
+ 弹性子元素的宽度在超过弹性容器宽度时, 会自适应缩减尺寸, 以防止溢出
```CSS
/** 假设有5个弹性子元素, 会缩减尺寸, 弹性子元素不会溢出 */
div.flex { display: flex; width: 800px; }
div.flex-item { width: 200px; }
```
+ 精细化控制单个弹性子元素的宽度变化: 使用`flex`属性, 只能用于弹性子元素
  - `flex`是简写属性, 包含`flex-grow`, `flex-shrink`, `flex-basis`属性
  - `flex-grow`: 弹性子元素宽度的增长因子, 默认值为`0`, 即宽度绝不会增加
  - `flex-shrink`: 弹性子元素宽度的缩减因子, 默认值为`1`, 即宽度会缩减
  - `flex-basis`: 弹性子元素宽度的基准值, 增长/缩减因子根据基准值计算缩放
***
**注解1:** 首先解释基准值, 稍后再分别讨论增长因子和缩减因子, 根据以下CSS规则
+ 弹性容器的宽度是`600px`
+ 每个弹性子元素既不会增大宽度, 也不会缩减宽度, 而是固定在`200px`
+ 弹性子元素会向弹性容器右侧溢出
+ **解释:** 基准值, 就是给弹性子元素设置的初始基准宽度
```CSS
/** 假设有5个弹性子元素 */
...
+ 增长因子: `flex-grow`, 即`flex`的第一个属性值
  - `flex-grow`不能设置负数
  - `flex-grow`设置弹性子元素是否允许放大宽度
  - `flex-grow`设置弹性子元素相比于同辈其他弹性子元素放大多少宽度
```CSS
/** 假设有2个弹性子元素 */
div.flex { display: flex; width: 600px; flex-wrap: nowrap; }
...
+ 剩余宽度 = 弹性容器内容区域宽度 - 所有弹性子元素实际占用宽度
***
+ 缩减因子: `flex-shrink`, 即`flex`的第二个属性值
  - `flex-shrink`不能设置负数
  - `flex-shrink`设置弹性子元素是否允许缩减宽度
  - `flex-shrink`设置弹性子元素相比于同辈其他弹性子元素缩减多少宽度
+ 缩减因子的用法和增长因子完全一致, 此处就不再给出示例代码了
***
**注解1:** 只有弹性子元素溢出时才需要缩减, 因此这里不再是剩余宽度了, 而是溢出宽度

**注解2:** 总结缩减因子
+ 设置为`0`, 则宽度不允许缩减, 尺寸固定在初始宽度
+ 设置为非`0`正数, 宽度允许缩减, 并和其他兄弟按比例分配溢出宽度
+ 溢出宽度 = 所有弹性子元素实际占用宽度 - 弹性容器内容区域宽度
***
+ 本小节主要讲述了弹性子元素自身尺寸的放大或缩减, 最后讲述几个补充项
  - `flex-basis`可以设置为`content`, 表示基准值实时同步弹性子元素内容区域
  - `flex-basis`可以设置为`auto`, 表示基准值实时同步弹性子元素的`width`
  - `flex-basis`可以设置为`width`属性支持的其他单位, 计算方法也一致
  - `flex`属性支持进一步简写
+ `flex`属性的简写赋值
  - `initial`: 弹性子元素确定宽度后, 允许缩小, 不允许放大
  - `auto`: 弹性子元素确定宽度后, 允许缩小, 也允许放大
  - `none`: 弹性子元素确定宽度后, 不允许缩小, 不允许放大
  - 数字值: 将增长因子设为该值, 缩减因子设为0, 允许放大, 不允许缩小
***
**总结:** 本小节介绍了如下CSS属性
+ `flex-grow`: 弹性子元素宽度是否允许放大和放大多少, 只能用于弹性子元素
+ `flex-shrink`: 弹性子元素宽度是否允许缩减和缩减多少, 只能用于弹性子元素
+ `flex-basis`: 弹性子元素的基准宽度值, 只能用于弹性子元素
+ `flex`: 上述三个属性的简写属性, 只能用于弹性子元素
***
