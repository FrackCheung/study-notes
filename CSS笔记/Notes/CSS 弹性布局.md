...
+ `flex-start`: 弹性子元素向右溢出
+ `flex-end`: 弹性子元素向左溢出
+ `center`: 弹性子元素向两端溢出, 溢出量相等
+ `space-between`: 弹性子元素向右溢出
+ `space-around`: 弹性子元素向两端溢出, 溢出量相等
+ `space-evenly`: 弹性子元素向两端溢出, 溢出量相等
***
+ 弹性子元素在主轴上的对齐: 使用`align-items`属性, 只能用于弹性容器
  - `flex-start`: 所有弹性子元素顶端对齐
  - `flex-end`: 所有弹性子元素底端对齐
  - `center`: 所有弹性子元素水平居中对齐
  - `baseline`: 所有弹性子元素的基线对齐
  - `stretch`: 默认值, 所有弹性子元素被拉伸至同一高度
***
**注解1:** 关于`baseline`基线
...
**注解2:** 所有弹性子元素行超高时
+ `flex-start`: 弹性子元素行向下溢出
+ `flex-end`: 弹性子元素行向上溢出
+ `center`: 弹性子元素行向两端溢出, 溢出量相等
+ `space-between`: 弹性子元素行向下溢出
