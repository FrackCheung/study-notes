...
+ `var`可以反复重复定义相同名称的变量
  ...
  var name = 'wangwu';
  ```
+ `var`定义的全局变量会成为`global`对象的属性(`let/const`也一样)
  ```JavaScript
  var name = 'zhangzhen';
...
+ 宿主对象: 由宿主环境(浏览器, Nodejs等)提供给JavaScript使用的对象: `window`/`global`/`document`等
  - 无法访问`Object`内置方法
  - 如需使用`Object`内置方法, 需使用`Object.prototype.method.call/apply`形式
  - 更详细的宿主对象信息, 将在`BOM`中讲述
...
+ 原型`prototype`
  ...
  - 函数才有`prototype`属性, 普通对象只有一个内部原型指针, 指向构造函数的原型对象, 来引用原型属性和方法
  - 上述两点的关系如下
  ```mermaid
  graph TD
  ...
  ```
+ 对象的原型指针不能直接获取, 可以通过`Object.getPrototypeOf`方法获取
